...
<input id="rep-param-id-${param.id}" type="text" class="form-control" inputmode="numeric" autocomplete="off">
      <label class="input-group-text btn-calendar"><i class="fa fa-calendar fa-sm"></i></label>
                ...
                ForeignKey(param) {
                    ...
                    let filter = param.info.filter;
                    if (Katrid.isString(filter))
                        filter = eval(`(${filter})`);
                    filter = param.info.filter ? ` filter='${JSON.stringify(filter)}'` : '';
...
`;
            ...
            const recCount = `<span class="badge text-bg-primary text-white" v-if="selectionLength" v-text="selectionLength.toString() + '/' + modelValue?.length"></span>`;
            let div = document.createElement('div');
