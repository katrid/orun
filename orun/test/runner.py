...
from orun.test import SimpleTestCase, TestCase
...
class DebugSQLTextTestResult(unittest.TextTestResult):
    def __init__(self, stream, descriptions, verbosity):
        self.logger = logging.getLogger('orun.db.backends')
...
def default_test_processes():
    ...
    if multiprocessing.get_start_method() != 'fork':
        return 1
    try:
        return int(os.environ['ORUN_TEST_PROCESSES'])
    except KeyError:
...
def _init_worker(counter):
    ...
    for alias in connections:
        ...
        # reflected in orun.db.connections. If the following line assigned
...
class DiscoverRunner:
    """A Orun test runner that uses unittest2 test discovery."""
    ...
    def run_tests(self, test_labels, extra_tests=None, **kwargs):
        ...
        run_failed = False
        try:
            # self.run_checks()  ## TODO enable it in future
