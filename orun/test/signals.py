...
from orun.dispatch import Signal, receiver
from orun.utils import timezone
from orun.utils.formats import FORMAT_SETTINGS, reset_format_cache
...
def reset_template_engines(**kwargs):
    if kwargs['setting'] in {
        ...
        from orun.forms.renderers import get_default_renderer
...
def user_model_swapped(**kwargs):
    if kwargs['setting'] == 'AUTH_USER_MODEL':
        apps.clear_cache()
        try:
            from orun.contrib.auth import get_user_model
            UserModel = get_user_model()
        except ImproperlyConfigured:
            # Some tests set an invalid AUTH_USER_MODEL.
            pass
        else:
            from orun.contrib.auth import backends
            backends.UserModel = UserModel

            from orun.contrib.auth import forms
            forms.UserModel = UserModel

            from orun.contrib.auth.handlers import modwsgi
            modwsgi.UserModel = UserModel

            from orun.contrib.auth.management.commands import changepassword
            changepassword.UserModel = UserModel

            from orun.contrib.auth import views
