...
class Registry:
    ...
    def populate(self, installed_apps=None):
        ...
        with self._lock:
            ...
            for addon in installed_apps:
                ...
                if addon.schema in self.addons:
                    raise ImproperlyConfigured(
                        "Application schema aren't unique, "
                        "duplicates: %s" % addon.schema)

                self.addons[addon.schema] = addon
                addon.app = self

            # Check for duplicate app names.
            counts = Counter(
                app_config.name for app_config in self.addons.values())
            duplicates = [
    ...
    def set_available_apps(self, available):
        ...
        installed = {app_config.name for app_config in self.get_addons()}
        ...
        self.stored_addons.append(self.addons)
        self.app_configs = OrderedDict(
            (label, app_config)
            for label, app_config in self.addons.items()
            if app_config.name in available)
        self.clear_cache()

    def unset_available_apps(self):
        """Cancel a previous call to set_available_apps()."""
        self.addons = self.stored_addons.pop()
        self.clear_cache()
    ...
    def is_installed(self, app_name):
        ...
        """
        self.check_apps_ready()
        return any(ac.name == app_name for ac in self.addons.values())
