...
from orun.utils import datetime_safe
from orun.utils.encoding import force_text
from orun.utils.functional import LazyObject, Promise
...
class DatetimeSerializer(BaseSerializer):
    def serialize(self):
        ...
        if self.value.tzinfo is not None:
            imports.append("from orun.utils.timezone import utc")
    ...
    def serialize(self):
        ...
        if not hasattr(module, self.value.__name__):
            raise ValueError(
                ...
                "https://docs.orunproject.com/en/%s/topics/migrations/#serializing-values"
...
class SettingsReferenceSerializer(BaseSerializer):
    def serialize(self):
        return "settings.%s" % self.value.setting_name, {"from orun.conf import settings"}
...
def serializer_factory(value):
    from orun.db.migrations.writer import SettingsReference
    ...
    raise ValueError(
        "Cannot serialize: %r\nThere are some values Orun cannot serialize into "
        "migration files.\nFor more, see https://docs.katrid.com/orun/en/%s/"
