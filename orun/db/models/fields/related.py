...
from orun.db import connection, router
...
def resolve_relation(scope_model, relation):
    ...
    if isinstance(relation, str):
        if "." not in relation:
            relation = "%s.%s" % (scope_model._meta.schema, relation)

    return relation


def lazy_related_operation(function, model, related_model, *args, **kwargs):
    """
    ...
    if related_model == 'self':
        related_model = model
    return apps.lazy_model_operation(partial(function, model, *args, **kwargs), make_model_name(related_model))
...
class ForeignKey(ForeignObject):
    ...
    def get_select_related(self, only: List[str], path=None) -> List[str]:
        ...
        if path is None:
            path = self.name + '__'
        if name_fields:
            name_fields = [self.remote_field.model._meta.fields[f] for f in name_fields]
        else:
            name_fields = self.remote_field.model._meta.get_name_fields()
        for f in name_fields:
            if f.many_to_one:
                res.append(f'{path or self.name}{f.name}')
    ...
    def _formfield(self):
        ...
        res['model'] = self.remote_field.model if isinstance(self.remote_field.model, str) else self.remote_field.model._meta.name
        if self.filter:
            res['filter'] = json.dumps(self.filter) if isinstance(self.filter, dict) else self.filter
        return res
...
def create_many_to_many_intermediary_model(field, klass):
    ...
    meta = type('Meta', (), {
        ...
        'name': '%s.%s.rel' % (klass._meta.name, field.name),
        'auto_created': klass,
    ...
    new_model = type(name, (models.Model,), {
        'Meta': meta,
                ...
                if seen_self > 2 and not self.remote_field.through_fields:
                    errors.append(
                        checks.Error(
                            ...
                            "which two foreign keys Orun should use via the "
                            "through_fields keyword argument." % (self, from_model_name),
                            hint="Use through_fields to specify which two foreign keys Orun should use.",
                    ...
                    errors.append(
                        checks.Error(
                            ("The model is used as an intermediate model by "
                             ...
                             "which foreign key Orun should use via the "
                             "through_fields keyword argument.") % (self, from_model_name),
                            hint=(
...
class OneToManyField(RelatedField):
    ...
    def contribute_to_class(self, cls, name):
        ...
        setattr(cls, name, self.remote_field)
