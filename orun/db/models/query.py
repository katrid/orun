...
from orun.db import (
    ORUN_VERSION_PICKLE_KEY, IntegrityError, connections, router,
    transaction,
)
from orun.db.models import DateField, DateTimeField, sql
from orun.db.models.constants import LOOKUP_SEP
from orun.db.models.deletion import Collector
from orun.db.models.expressions import Case, Expression, F, Value, When
from orun.db.models.fields import AutoField
from orun.db.models.functions import Cast, Trunc
from orun.db.models.query_utils import FilteredRelation, InvalidQuery, Q
from orun.db.models.sql.constants import CURSOR, GET_ITERATOR_CHUNK_SIZE
...
class QuerySet:
    ...
    def __getstate__(self):
        ...
        self._fetch_all()
        data = {**self.__dict__, ORUN_VERSION_PICKLE_KEY: get_version()}
        data.pop('model')
        return data

    def __reduce__(self):
        data = self.__getstate__()
        class_id = self.model._meta.name
        return queryset_unpickle, (class_id,), data
    ...
    def __setstate__(self, state):
        ...
        if pickled_version:
            ...
            if current_version != pickled_version:
                msg = (
                    "Pickled queryset instance's Orun version %s does not "
                    "match the current version %s." % (pickled_version, current_version)
                )
        else:
            msg = "Pickled queryset instance's Orun version is not specified."
            ...
            for obj in obj_list:
                ...
                if not hasattr(obj, '_prefetched_objects_cache'):
                    try:
                        ...
                        # instances (AttributeError), either in Orun or a 3rd
...
def queryset_unpickle(model_id):
    """Used to unpickle Model subclasses with deferred fields."""
    return QuerySet(apps[model_id])
