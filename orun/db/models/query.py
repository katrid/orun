...
from orun.db.models.expressions import Case, Expression, F, Ref, Value, When
from orun.db.models.functions import Cast, Trunc
from orun.db.models.query_utils import FilteredRelation, Q
from orun.db.models.sql.constants import CURSOR, GET_ITERATOR_CHUNK_SIZE
from orun.db.models.utils import create_namedtuple_class, resolve_callables
...
class QuerySet:
    ...
    def __getstate__(self):
        ...
        self._fetch_all()
        return {**self.__dict__, ORUN_VERSION_PICKLE_KEY: orun.__version__}

    def __setstate__(self, state):
        pickled_version = state.get(ORUN_VERSION_PICKLE_KEY)
        if pickled_version:
            if pickled_version != orun.__version__:
                warnings.warn(
                    "Pickled queryset instance's Orun version %s does not "
                    "match the current version %s."
                    % (pickled_version, orun.__version__),
                    RuntimeWarning,
                    stacklevel=2,
                )
        else:
            warnings.warn(
                "Pickled queryset instance's Orun version is not specified.",
                RuntimeWarning,
                stacklevel=2,
            )
    ...
    def _annotate(self, args, kwargs, select=True):
        ...
        if names is None:
            names = set(chain.from_iterable(
                ...
                for field in self.model._meta.get_fields() if field.concrete
            ))
