...
from orun.conf.locale import LANG_INFO
...
class OrunTranslation(gettext_module.GNUTranslations):
    """
    ...
    def __init__(self, language, domain=None, localedirs=None):
        ...
        if self.domain == 'orun':
            if localedirs is not None:
                # A module-level cache is used for caching 'orun' translations
                warnings.warn("localedirs is ignored when domain is 'orun'.", RuntimeWarning)
                localedirs = None
        ...
        if self.__language == settings.LANGUAGE_CODE and self.domain == 'orun' and self._catalog is None:
    ...
    def __repr__(self):
        return "<OrunTranslation lang:%s>" % self.__language
    ...
    def _init_translation_catalog(self):
        """Creates a base catalog using global orun translations."""
    ...
    def _add_installed_apps_translations(self):
        ...
        try:
            app_configs = reversed(list(apps.app_configs.values()))
    ...
    def _add_fallback(self, localedirs=None):
        ...
        if self.domain == 'orun':
            ...
            default_translation = OrunTranslation(
    ...
    def merge(self, other):
        ...
        if self._catalog is None:
            # Take plural and _info from first catalog found (generally Orun's).
...
def translation(language):
    """
    Returns a translation object in the default 'orun' domain.
    """
    global _translations
    if language not in _translations:
        _translations[language] = OrunTranslation(language)
...
def check_for_language(lang_code):
    ...
    <https://www.orunproject.com/weblog/2007/oct/26/security-fix/>.
    """
    # First, a quick check to make sure lang_code is well-formed (#21458)
    if lang_code is None or not language_code_re.search(lang_code):
        return False
    for path in all_locale_paths():
        if gettext_module.find('orun', path, [to_locale(lang_code)]) is not None:
            return True
    return False
...
def templatize(src, origin=None):
    """
    Turns a Orun template into something that is understood by xgettext. It
    does so by translating the Orun translation tags into standard gettext
    function invocations.
    """
    from orun.template.base import (
