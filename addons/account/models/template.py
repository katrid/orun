...
class AccountAccountTemplate(models.Model):
    ...
    currency_id = models.ForeignKey('res.currency', verbose_name='Account Currency', help_text="Forces all moves for this account to have this secondary currency.")
    code = models.CharField(size=64, null=False, db_index=True)
    user_type_id = models.ForeignKey('account.account.type', verbose_name='Type', null=False,
                                   help_text="These types are defined according to your country. The type contains more information " \
                                        "about the account and its specificities.")
    reconcile = models.BooleanField(verbose_name='Allow Invoices & payments Matching', default=False,
                               help_text="Check this option if you want the user to reconcile entries in this account.")
    note = models.TextField()
    tax_ids = models.ManyToManyField('account.tax.template', 'account_account_template_tax_rel', 'account_id', 'tax_id', verbose_name='Default Taxes')
    nocreate = models.BooleanField(verbose_name='Optional Create', default=False,
                              help_text="If checked, the new chart of accounts will not contain this by default.")
    chart_template_id = models.ForeignKey('account.chart.template', verbose_name='Chart Template',
                                        help_text="This optional field allow you to link an account template to a specific chart template that may differ from the one its root parent belongs to. This allow you "
    ...
    tag_ids = models.ManyToManyField('account.account.tag', 'account_account_template_account_tag', verbose_name='Account tag', help_text="Optional tags you may want to assign for custom reporting")
    group_id = models.ForeignKey('account.group')
    root_id = models.ForeignKey('account.root')
    ...
    class Meta:
        ...
        description = 'Templates for Accounts'
        ordering = "code"
...
class AccountChartTemplate(models.Model):
    ...
    parent_id = models.ForeignKey('account.chart.template', verbose_name='Parent Chart Template')
    code_digits = models.IntegerField(verbose_name='# of Digits', null=False, default=6, help_text="No. of Digits to use for account code")
    visible = models.BooleanField(verbose_name='Can be Visible?', default=True,
                             help_text="Set this to False if you don't want this template to be used actively in the wizard that generate Chart of Accounts from "
                                  "templates, this is useful when you want to generate accounts of this template only when loading its child template.")
    currency_id = models.ForeignKey('res.currency', verbose_name='Currency', null=False)
    use_anglo_saxon = models.BooleanField(verbose_name="Use Anglo-Saxon accounting", default=False)
    complete_tax_set = models.BooleanField(verbose_name='Complete Set of Taxes', default=True,
                                      help_text="This boolean helps you to choose if you want to propose to the user to encode the sale and purchase rates or choose from list "
                                           "of taxes. This last choice assumes that the set of tax defined on this template is complete")
    account_ids = models.OneToManyField('account.account.template', 'chart_template_id', verbose_name='Associated Account Templates')
    tax_template_ids = models.OneToManyField('account.tax.template', 'chart_template_id', verbose_name='Tax Template List',
                                       help_text='List of all the taxes that have to be installed by the wizard')
    bank_account_code_prefix = models.CharField(verbose_name='Prefix of the bank accounts', null=False)
    cash_account_code_prefix = models.CharField(verbose_name='Prefix of the main cash accounts', null=False)
    transfer_account_code_prefix = models.CharField(verbose_name='Prefix of the main transfer accounts', null=False)
    income_currency_exchange_account_id = models.ForeignKey('account.account.template',
                                                          verbose_name="Gain Exchange Rate Account", domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)])
    expense_currency_exchange_account_id = models.ForeignKey('account.account.template',
                                                           verbose_name="Loss Exchange Rate Account", domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)])
    default_cash_difference_income_account_id = models.ForeignKey('account.account.template', verbose_name="Cash Difference Income Account")
    default_cash_difference_expense_account_id = models.ForeignKey('account.account.template', verbose_name="Cash Difference Expense Account")
    default_pos_receivable_account_id = models.ForeignKey('account.account.template', verbose_name="PoS receivable account")
    property_account_receivable_id = models.ForeignKey('account.account.template', verbose_name='Receivable Account')
    property_account_payable_id = models.ForeignKey('account.account.template', verbose_name='Payable Account')
    property_account_expense_categ_id = models.ForeignKey('account.account.template', verbose_name='Category of Expense Account')
    property_account_income_categ_id = models.ForeignKey('account.account.template', verbose_name='Category of Income Account')
    property_account_expense_id = models.ForeignKey('account.account.template', verbose_name='Expense Account on Product Template')
    property_account_income_id = models.ForeignKey('account.account.template', verbose_name='Income Account on Product Template')
    property_stock_account_input_categ_id = models.ForeignKey('account.account.template', verbose_name="Input Account for Stock Valuation")
    property_stock_account_output_categ_id = models.ForeignKey('account.account.template', verbose_name="Output Account for Stock Valuation")
    property_stock_valuation_account_id = models.ForeignKey('account.account.template', verbose_name="Account Template for Stock Valuation")
    property_tax_payable_account_id = models.ForeignKey('account.account.template', verbose_name="Tax current account (payable)")
    property_tax_receivable_account_id = models.ForeignKey('account.account.template', verbose_name="Tax current account (receivable)")
    property_advance_tax_payment_account_id = models.ForeignKey('account.account.template', verbose_name="Advance tax payment account")
...
class AccountTaxTemplate(models.Model):
    ...
    chart_template_id = models.ForeignKey('account.chart.template', verbose_name='Chart Template', null=False)

    name = models.CharField(verbose_name='Tax Name', null=False)
    type_tax_use = models.ChoiceField(TYPE_TAX_USE, verbose_name='Tax Scope', null=False, default="sale",
                                    help_text="Determines where the tax is selectable. Note : 'None' means a tax can't be used by itself, however it can still be used in a group.")
    amount_type = models.ChoiceField(default='percent', verbose_name="Tax Computation", null=False,
                                   selection=[('group', 'Group of Taxes'), ('fixed', 'Fixed'), ('percent', 'Percentage of Price'), ('division', 'Percentage of Price Tax Included')])
    active = models.BooleanField(default=True, help_text="Set active to false to hide the tax without removing it.")
    children_tax_ids = models.ManyToManyField('account.tax.template', 'account_tax_template_filiation_rel', 'parent_tax', 'child_tax', verbose_name='Children Taxes')
    sequence = models.IntegerField(null=False, default=1,
                              help_text="The sequence field is used to define order in which the tax lines are applied.")
    amount = models.FloatField(null=False, digits=(16, 4), default=0)
    description = models.CharField(verbose_name='Display on Invoices')
    price_include = models.BooleanField(verbose_name='Included in Price', default=False,
                                   help_text="Check this if the price you use on the product and invoices includes this tax.")
    include_base_amount = models.BooleanField(verbose_name='Affect Subsequent Taxes', default=False,
                                         help_text="If set, taxes which are computed after this one will be computed based on the price tax included.")
    analytic = models.BooleanField(verbose_name="Analytic Cost", help_text="If set, the amount computed by this tax will be assigned to the same analytic account as the invoice line (if any)")
    invoice_repartition_line_ids = models.OneToManyField(verbose_name="Repartition for Invoices", comodel_name="account.tax.repartition.line.template", inverse_name="invoice_tax_id", copy=True, help_text="Repartition when the tax is used on an invoice")
    refund_repartition_line_ids = models.OneToManyField(verbose_name="Repartition for Refund Invoices", comodel_name="account.tax.repartition.line.template", inverse_name="refund_tax_id", copy=True, help_text="Repartition when the tax is used on a refund")
    tax_group_id = models.ForeignKey('account.tax.group', verbose_name="Tax Group")
    tax_exigibility = models.ChoiceField(
        [('on_invoice', 'Based on Invoice'),
         ('on_payment', 'Based on Payment'),
         ], verbose_name='Tax Due', default='on_invoice',
        help_text="Based on Invoice: the tax is due as soon as the invoice is validated.\n"
             "Based on Payment: the tax is due as soon as the payment of the invoice is received.")
    cash_basis_transition_account_id = models.ForeignKey(
        comodel_name='account.account.template',
        verbose_name="Cash Basis Transition Account",
        domain=[('deprecated', '=', False)],
        help_text="Account used to transition the tax amount for cash basis taxes. It will contain the tax amount as long as the original invoice has not been reconciled ; at reconciliation, this amount cancelled on this account and put on the regular tax account.")
    cash_basis_base_account_id = models.ForeignKey(
        'account.account.template',
        domain=[('deprecated', '=', False)],
        verbose_name='Base Tax Received Account',
        help_text='Account that will be set on lines created in cash basis journal entry and used to keep track of the tax base amount.')
...
class AccountTaxRepartitionLineTemplate(models.Model):
    ...
    factor_percent = models.FloatField(verbose_name="%", null=False, help_text="Factor to apply on the account move lines generated from this repartition line, in percents")
    repartition_type = models.ChoiceField(verbose_name="Based On", selection=[('base', 'Base'), ('tax', 'of tax')], null=False, default='tax', help_text="Base on which the factor will be applied.")
    account_id = models.ForeignKey(verbose_name="Account", comodel_name='account.account.template', help_text="Account on which to post the tax amount")
    invoice_tax_id = models.ForeignKey(comodel_name='account.tax.template', help_text="The tax set to apply this repartition on invoices. Mutually exclusive with refund_tax_id")
    refund_tax_id = models.ForeignKey(comodel_name='account.tax.template', help_text="The tax set to apply this repartition on refund invoices. Mutually exclusive with invoice_tax_id")
    tag_ids = models.ManyToManyField(verbose_name="Financial Tags", relation='account_tax_repartition_financial_tags', comodel_name='account.account.tag', copy=True, help_text="Additional tags that will be assigned by this repartition line for use in financial reports")
    ...
    plus_report_line_ids = models.ManyToManyField(verbose_name="Plus Tax Report Lines", relation='account_tax_repartition_plus_report_line', comodel_name='account.tax.report.line', copy=True, help_text="Tax report lines whose '+' tag will be assigned to move lines by this repartition line")
    minus_report_line_ids = models.ManyToManyField(verbose_name="Minus Report Lines", relation='account_tax_repartition_minus_report_line', comodel_name='account.tax.report.line', copy=True, help_text="Tax report lines whose '-' tag will be assigned to move lines by this repartition line")
...
class AccountFiscalPositionTemplate(models.Model):
    ...
    name = models.CharField(verbose_name='Fiscal Position Template', null=False)
    chart_template_id = models.ForeignKey('account.chart.template', verbose_name='Chart Template', null=False)
    account_ids = models.OneToManyField('account.fiscal.position.account.template', 'position_id', verbose_name='Account Mapping')
    tax_ids = models.OneToManyField('account.fiscal.position.tax.template', 'position_id', verbose_name='Tax Mapping')
    note = models.TextField(verbose_name='Notes')
    auto_apply = models.BooleanField(verbose_name='Detect Automatically', help_text="Apply automatically this fiscal position.")
    vat_required = models.BooleanField(verbose_name='VAT required', help_text="Apply only if partner has a VAT number.")
    country_id = models.ForeignKey('res.country', verbose_name='Country',
                                 help_text="Apply only if delivery or invoicing country match.")
    country_group_id = models.ForeignKey('res.country.group', verbose_name='Country Group',
                                       help_text="Apply only if delivery or invoicing country match the group.")
    state_ids = models.ManyToManyField('res.country.state', verbose_name='Federal States')
    zip_from = models.CharField(verbose_name='Zip Range From')
    zip_to = models.CharField(verbose_name='Zip Range To')
...
class AccountFiscalPositionTaxTemplate(models.Model):
    ...
    position_id = models.ForeignKey('account.fiscal.position.template', verbose_name='Fiscal Position', null=False, ondelete='cascade')
    tax_src_id = models.ForeignKey('account.tax.template', verbose_name='Tax Source', null=False)
    tax_dest_id = models.ForeignKey('account.tax.template', verbose_name='Replacement Tax')
...
class AccountFiscalPositionAccountTemplate(models.Model):
    ...
    position_id = models.ForeignKey('account.fiscal.position.template', verbose_name='Fiscal Mapping', null=False, ondelete='cascade')
    account_src_id = models.ForeignKey('account.account.template', verbose_name='Account Source', null=False)
    account_dest_id = models.ForeignKey('account.account.template', verbose_name='Account Destination', null=False)


class ReconcileModelTemplate(models.Model):
    # Base fields.
    chart_template_id = models.ForeignKey('account.chart.template', verbose_name='Chart Template', null=False)
    name = models.CharField(verbose_name='Button Label', null=False)
    sequence = models.IntegerField(null=False, default=10)

    rule_type = models.ChoiceField(selection=[
    ...
    ], verbose_name='Type', default='writeoff_button', null=False)
    auto_reconcile = models.BooleanField(verbose_name='Auto-validate',
                                    help_text='Validate the statement line automatically (reconciliation based on your rule).')
    to_check = models.BooleanField(verbose_name='To Check', default=False, help_text='This matching rule is used when the user is not certain of all the informations of the counterpart.')

    # ===== Conditions =====
    match_journal_ids = models.ManyToManyField('account.journal', verbose_name='Journals',
                                         domain="[('type', 'in', ('bank', 'cash'))]",
                                         help_text='The reconciliation model will only be available from the selected journals.')
    match_nature = models.ChoiceField(selection=[
        ('amount_received', 'Amount Received'),
        ('amount_paid', 'Amount Paid'),
        ('both', 'Amount Paid/Received')
    ], verbose_name='Amount Nature', null=False, default='both',
        help_text='''The reconciliation model will only be applied to the selected transaction type:
        * Amount Received: Only applied when receiving an amount.
        * Amount Paid: Only applied when paying an amount.
        * Amount Paid/Received: Applied in both cases.''')
    match_amount = models.ChoiceField(selection=[
        ('lower', 'Is Lower Than'),
        ('greater', 'Is Greater Than'),
        ('between', 'Is Between'),
    ], verbose_name='Amount',
        help_text='The reconciliation model will only be applied when the amount being lower than, greater than or between specified amount(s).')
    match_amount_min = models.FloatField(verbose_name='Amount Min Parameter')
    match_amount_max = models.FloatField(verbose_name='Amount Max Parameter')
    match_label = models.ChoiceField(selection=[
        ('contains', 'Contains'),
        ('not_contains', 'Not Contains'),
        ('match_regex', 'Match Regex'),
    ], verbose_name='Label', help_text='''The reconciliation model will only be applied when the label:
        * Contains: The proposition label must contains this string (case insensitive).
        * Not Contains: Negation of "Contains".
        * Match Regex: Define your own regular expression.''')
    match_label_param = models.CharField(verbose_name='Label Parameter')
    match_note = models.ChoiceField(selection=[
        ('contains', 'Contains'),
        ('not_contains', 'Not Contains'),
        ('match_regex', 'Match Regex'),
    ], verbose_name='Note', help_text='''The reconciliation model will only be applied when the note:
        * Contains: The proposition note must contains this string (case insensitive).
        * Not Contains: Negation of "Contains".
        * Match Regex: Define your own regular expression.''')
    match_note_param = models.CharField(verbose_name='Note Parameter')
    match_transaction_type = models.ChoiceField(selection=[
        ('contains', 'Contains'),
        ('not_contains', 'Not Contains'),
        ('match_regex', 'Match Regex'),
    ], verbose_name='Transaction Type', help_text='''The reconciliation model will only be applied when the transaction type:
        * Contains: The proposition transaction type must contains this string (case insensitive).
        * Not Contains: Negation of "Contains".
        * Match Regex: Define your own regular expression.''')
    match_transaction_type_param = models.CharField(verbose_name='Transaction Type Parameter')
    match_same_currency = models.BooleanField(verbose_name='Same Currency Matching', default=True,
                                         help_text='Restrict to propositions having the same currency as the statement line.')
    match_total_amount = models.BooleanField(verbose_name='Amount Matching', default=True,
                                        help_text='The sum of total residual amount propositions matches the statement line amount.')
    match_total_amount_param = models.FloatField(
        verbose_name='Amount Matching %', default=100,
        help_text='The sum of total residual amount propositions matches the statement line amount under this percentage.'
    )
    match_partner = models.BooleanField(verbose_name='Partner Is Set',
                                   help_text='The reconciliation model will only be applied when a customer/vendor is set.')
    match_partner_ids = models.ManyToManyField('res.partner', verbose_name='Restrict Partners to',
                                         help_text='The reconciliation model will only be applied to the selected customers/vendors.')
    match_partner_category_ids = models.ManyToManyField('res.partner.category', verbose_name='Restrict Partner Categories to',
                                                  help_text='The reconciliation model will only be applied to the selected customer/vendor categories.')

    # First part fields.
    account_id = models.ForeignKey('account.account.template', verbose_name='Account', ondelete='cascade', domain=[('deprecated', '=', False)])
    label = models.CharField(verbose_name='Journal Item Label')
    amount_type = models.ChoiceField(
        (
            ('$', 'Fixed'),
            ('%', 'Percentage of balance'),
            ('regex', 'From label'),
        ), null=False, default='percentage'
    )
    amount = models.FloatField(verbose_name='Write-off Amount', digits=0, null=False, default=100.0, help_text="Fixed amount will count as a debit if it is negative, as a credit if it is positive.")
    amount_from_label_regex = models.CharField(verbose_name="Amount from Label (regex)", default=r"([\d\.,]+)")
    decimal_separator = models.CharField(help_text="Every character that is nor a digit nor this separator will be removed from the matching string")
    force_tax_included = models.BooleanField(verbose_name='Tax Included in Price',
                                        help_text='Force the tax to be managed as a price included tax.')
    # Second part fields.
    has_second_line = models.BooleanField(verbose_name='Add a second line', default=False)
    tax_ids = models.ManyToManyField('account.tax.template', verbose_name='Taxes', ondelete='restrict')
    second_account_id = models.ForeignKey('account.account.template', verbose_name='Second Account', ondelete='cascade', domain=[('deprecated', '=', False)])
    second_label = models.CharField(verbose_name='Second Journal Item Label')
    second_amount_type = models.ChoiceField([
        ('$', 'Fixed'),
        ('%', 'Percentage of amount'),
        ('regex', 'From label'),
    ], verbose_name="Second Amount type",null=False, default='percentage')
    second_amount = models.FloatField(verbose_name='Second Write-off Amount', null=False, default=100.0, help_text="Fixed amount will count as a debit if it is negative, as a credit if it is positive.")
    second_amount_from_label_regex = models.CharField(verbose_name="Second Amount from Label (regex)", default=r"([\d\.,]+)")
    force_second_tax_included = models.BooleanField(
        verbose_name='Second Tax Included in Price',
        help_text='Force the second tax to be managed as a price included tax.'
    )
    second_tax_ids = models.ManyToManyField('account.tax.template', relation='account_reconcile_model_tmpl_account_tax_bis_rel', verbose_name='Second Taxes', ondelete='restrict')

    class Meta:
        name = "account.reconcile.model.template"
        verbose_name = _('Reconcile Model Template')
