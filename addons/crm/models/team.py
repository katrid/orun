...
class Team(sales_team.models.Team):
    ...
    use_leads = models.BooleanField('Leads', help_text="Check this box to filter and qualify incoming requests as leads before converting them into opportunities and assigning them to a salesperson.")
    use_opportunities = models.BooleanField('Pipeline', help_text="Check this box to manage a presales process with opportunities.")
    alias_id = models.ForeignKey('mail.alias', verbose_name='Alias', ondelete="restrict", required=True, help_text="The email address associated with this channel. New emails received will automatically create new leads assigned to the channel.")
    unassigned_leads_count = models.IntegerField(
        compute='_compute_unassigned_leads_count',
        verbose_name='Unassigned Leads', readonly=True)
    opportunities_count = models.IntegerField(
        compute='_compute_opportunities',
        verbose_name='Number of open opportunities', readonly=True)
    opportunities_amount = models.IntegerField(
        compute='_compute_opportunities',
        verbose_name='Opportunities Revenues', readonly=True)
    dashboard_graph_model = models.ChoiceField(add_choices=(('crm.lead', 'Pipeline'),))
    dashboard_graph_period_pipeline = models.ChoiceField([
        ('week', 'Within a Week'),
        ('month', 'Within a Month'),
        ('year', 'Within a Year'),
    ], verbose_name='Expected to Close', help_text="The time period this channel's dashboard graph will consider.",
    ...
    dashboard_graph_group_pipeline = models.ChoiceField([
        ...
        ('user', 'Salesperson'),
        ('stage', 'Stage'),
    ], verbose_name='Grouping Method', default='day', help_text="How this channel's dashboard graph will group the results.")
    ...
    def _compute_dashboard_button_name(self):
        ...
        opportunity_teams.update({'dashboard_button_name': gettext("Pipeline")})
