...
from orun.test import TestCase, TransactionTestCase
from orun.test.runner import default_test_processes
from orun.test.selenium import SeleniumTestCaseBase
from orun.test.utils import get_runner
from orun.utils.deprecation import (
    RemovedInOrun30Warning, RemovedInOrun31Warning,
)
...
warnings.simplefilter("error", RemovedInOrun30Warning)
warnings.simplefilter('error', RemovedInOrun31Warning)
...
TMPDIR = tempfile.mkdtemp(prefix='orun_')
...
CONTRIB_TESTS_TO_APPS = {
    'flatpages_tests': 'orun.contrib.flatpages',
    'redirects_tests': 'orun.contrib.redirects',
}
...
def setup(verbosity, test_labels, parallel):
    ...
    if verbosity >= 1:
        msg = "Testing against Orun installed in '%s'" % os.path.dirname(orun.__file__)
    ...
    # tests.
    log_config['loggers']['orun']['level'] = 'ERROR'
    ...
    # orun.setup() so that connection.features.gis_enabled can be accessed
...
def actual_test_processes(parallel):
    if parallel == 0:
        # This doesn't work before orun.setup() on some databases.
...
def orun_tests(verbosity, interactive, failfast, keepdb, reverse,
    ...
    if not hasattr(settings, 'TEST_RUNNER'):
        settings.TEST_RUNNER = 'orun.test.runner.DiscoverRunner'
...
if __name__ == "__main__":
    ...
    if options.settings:
        os.environ['ORUN_SETTINGS_MODULE'] = options.settings
    else:
        os.environ.setdefault('ORUN_SETTINGS_MODULE', 'test_sqlite')
        options.settings = os.environ['ORUN_SETTINGS_MODULE']
